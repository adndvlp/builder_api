...
export default function DropboxCallback() {
  ...
  const [message, setMessage] = useState(
    "Procesando autenticación con Dropbox..."
  );
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      if (error) {
        setStatus("error");
        setMessage(`Error de Dropbox: ${error}`);
      ...
      try {
        ...
        const response = await fetch(
          `/api/dropboxOAuthCallback?code=${code}&state=${state}`
        );

        if (response.ok) {
          setStatus("success");
          setMessage("¡Dropbox conectado exitosamente! Redirigiendo...");
